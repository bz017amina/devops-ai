...
#  Phase 1 : Infrastructure as Code (AWS avec Terraform)

**Responsable :** Amina BOUAZZA

Cette partie du projet concerne la création automatisée et sécurisée de l'infrastructure sur AWS. L'utilisation de **Terraform** permet de définir l'intégralité des ressources via du code, garantissant ainsi une infrastructure reproductible, scalable et documentée.

## 1. Description de l'Infrastructure

L'infrastructure est déployée dans la région `us-east-1` (Virginie du Nord) et repose sur un cluster Kubernetes léger (**K3s**) composé de deux instances EC2 stratégiques :

* **k3s-master** : Instance principale faisant office de **Control Plane**. Elle gère l'orchestration, l'ordonnancement des pods et l'état du cluster.
* **k3s-worker** : Instance nœud (**Worker Node**) dédiée exclusivement à l'exécution des conteneurs de l'application IA.

### Spécifications techniques :
* **AMI** : Amazon Linux 2 (`ami-0c02fb55956c7d316`).
* **Type d'instance Master** : `t3.small` (Choisi pour la stabilité du Control Plane).
* **Type d'instance Worker** : `t3.micro` (Éligible au Free Tier AWS).
* **Réseau** : VPC par défaut avec un **Security Group** (`k3s-security-group`) optimisé ouvrant les flux nécessaires :

| Service / Usage | Port | Protocole | Description |
| :--- | :--- | :--- | :--- |
| **SSH** | 22 | TCP | Administration et configuration via Ansible. |
| **Kubernetes API** | 6443 | TCP | Communication avec le serveur d'API. |
| **Kubelet** | 10250 | TCP | Gestion interne des nœuds. |
| **Flannel Networking**| 8472 | UDP | Communication inter-pods (VXLAN). |
| **Application (Prod)** | 32542 | TCP | Accès externe à l'application **CreditSim**. |
| **Monitoring / Dev** | 3000 / 32000 | TCP | Accès Grafana et tests de l'équipe. |

## 2.  Informations de Connexion

L'accès aux instances s'effectue via SSH en utilisant la clé privée sécurisée.

| Nom | Rôle | Instance Type | Adresse IP Publique | Utilisateur |
| :--- | :--- | :--- | :--- | :--- |
| **k3s-master** | Master Node | `t3.small` | `54.xx.xx.xx` | `ec2-user` |
| **k3s-worker** | Worker Node | `t3.micro` | `3.xx.xx.xx` | `ec2-user` |

> 🔒 **Sécurité** : Les adresses IP réelles sont masquées dans cette documentation publique. La connexion nécessite le fichier de clé privée `devops-key.pem` (non inclus dans le dépôt pour des raisons de sécurité).

## 3. Workflow et Commandes utilisées

Le cycle de vie de l'infrastructure est géré via les commandes standard de Terraform :

1.  **`terraform init`** : Initialisation du projet et téléchargement des providers AWS.
2.  **`terraform plan`** : Simulation pour visualiser les modifications et valider la configuration.
3.  **`terraform apply`** : Déploiement réel des ressources sur le Cloud AWS.
...
"Demo test - $(date)" 
Demo prof - 24/03/2026 22:44:37,28
